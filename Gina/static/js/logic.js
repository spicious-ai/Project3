 ...
 let myMap = L.map("map", {
  ...
  for (let i = 0; i < wildfireData.length; i++) {
    ...
    let discoveryDate = wildfireData[i].DISCOVERY_DATE;
    let containmentDate = wildfireData[i].CONT_DATE;    
    let daysOfFire = Math.round((containmentDate - discoveryDate) / (1000 * 60 * 60 * 24));
    let marker = L.circleMarker([lat, lon]);
    marker.bindPopup(`<b>Fire Name:</b> ${wildfireData[i].FIRE_NAME}<br><b>Fire Year:</b> ${wildfireData[i].FIRE_YEAR}<br><b>Cause:</b> ${wildfireData[i].NWCG_GENERAL_CAUSE}<br><b># of Days:</b> ${daysOfFire}`);
    fireMarkers.addLayer(marker);
    console.log(`Number of days of fire: ${daysOfFire}`);
  };
