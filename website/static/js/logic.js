...
var queryUrl = "http://localhost:8000/api/v1.0/fires.geojson";
var otherthing = "https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/all_month.geojson";
// Perform a GET request to the query URL/
d3.json(otherthing).then(function (data) {
...
function createFeatures(earthquakeData) {
  ...
  // Give each feature a popup that describes the place and time of the earthquake.
    ...
    console.log("hello from createfeatures");
  function onEachFeature(feature, layer) {
      console.log(feature)
      layer.bindPopup(`<h3>${feature.properties.cause}</h3><hr><p>${new Date(feature.properties.year)}</p>`);
  }

  // Create a GeoJSON layer that contains the features array on the earthquakeData object.
  // Run the onEachFeature function once for each piece of data in the array.
  var earthquakes = L.geoJSON(earthquakeData[0], {
  pointToLayer: (feature, latlng) => {
      //console.log(feature[0]);
      return new L.circleMarker(latlng, {
      fillOpacity: 1,
      color: sortColor(feature.geometry.size),
      fillColor: sortColor(feature.geometry.size),
